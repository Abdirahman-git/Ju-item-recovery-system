...
export default function RegisterScreen() {
  ...
  const handleValidateId = async () => {
    if (!studentId.trim()) {
      toastRef.current?.show('Enter your Student ID', '', 'error');
      return;
    }
  ...
  const handleSendOtp = async () => {
    if (!studentId.trim()) {
      toastRef.current?.show('Student ID missing. Go back and verify again.', '', 'error');
      return;
    }
    ...
    try {
      ...
      toastRef.current?.show(err.message || 'Failed to send OTP', '', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyOtp = async () => {
    if (otp.trim().length < 6) {
      toastRef.current?.show('Enter the 6-digit code', '', 'error');
      return;
    }
    ...
    try {
      ...
      toastRef.current?.show(err.message || 'Verification Failed', '', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleActivateAccount = async () => {
    if (!password || !confirmPassword) {
      toastRef.current?.show('Fill in password fields', '', 'error');
      return;
    }
    ...
    if (password.length < 6) {
      toastRef.current?.show('Password must be 6+ chars', '', 'error');
      return;
    }
    ...
    try {
      ...
      toastRef.current?.show(err.message || 'Activation Failed', '', 'error');
    } finally {
      setLoading(false);
    }
  };
